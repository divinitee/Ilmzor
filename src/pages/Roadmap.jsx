...
import { CURRENT_STAGE_INDEX, getRoadmapStages } from "@/content/roadmapJourney";
...
export default function Roadmap() {
  ...
  return <div className="landing-dark min-h-screen bg-[#050612] text-slate-50 overflow-x-hidden">
    ...
    <header className="relative z-10 max-w-6xl mx-auto px-5 py-5 flex items-center justify-between"><Link to="/landing" className="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-violet-300"><ArrowLeft className="w-4 h-4"/>Back to VIRORA</Link><div className="flex items-center gap-2"><div className="w-9 h-9 rounded-xl bg-violet-500/10 border border-violet-400/20 flex items-center justify-center"><BookOpen className="w-4 h-4 text-violet-300"/></div><span className="font-bold tracking-[0.18em] text-sm">VIRORA</span></div></header>
    <main className="relative z-10 max-w-6xl mx-auto px-5 pb-28">
      <section className="pt-14 sm:pt-20 text-center max-w-4xl mx-auto"><motion.div initial={{opacity:0,y:15}} animate={{opacity:1,y:0}} transition={{duration:.7}}><div className="inline-flex items-center gap-2 rounded-full border border-violet-400/20 bg-violet-500/[0.07] px-3.5 py-1.5 text-[10px] font-black tracking-[0.18em] text-violet-300"><Sparkles className="w-3.5 h-3.5"/>THE VIRORA JOURNEY</div><h1 className="mt-7 text-4xl sm:text-6xl lg:text-7xl font-bold tracking-[-0.035em]">You're here before the<br className="hidden sm:block"/> full system exists.</h1><p className="mt-6 text-base sm:text-lg leading-7 text-slate-400 max-w-2xl mx-auto">VIRORA is being built one layer at a time. Explore what already exists, what we're building now, and where the system is going.</p></motion.div></section>
        ...
        <motion.div layout className="mt-10 rounded-[2rem] border border-violet-400/15 bg-violet-500/[0.045] shadow-[0_20px_80px_rgba(0,0,0,.24)] overflow-hidden">
          <div className="grid lg:grid-cols-[1.15fr_.85fr]">
            ...
            <div className="border-t lg:border-t-0 lg:border-l border-violet-400/10 p-7 sm:p-10 bg-black/10">
              <div className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-600">Your position</div>
              <div className="mt-4 flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-violet-500/10 border border-violet-400/20 flex items-center justify-center"><LockKeyhole className="w-4 h-4 text-violet-300"/></div><div><div className="text-sm font-semibold">Founding Learner</div><div className="text-[11px] text-slate-500">Enter during this chapter and lock the current price.</div></div></div>
              <Link to="/pricing" className="mt-6 inline-flex items-center gap-2 text-xs font-bold text-violet-300 hover:text-violet-200">See founding pricing<ArrowRight className="w-3.5 h-3.5"/></Link>
            </div>
          </div>
        </motion.div>
      </section>

      <section className="mt-28"><div className="max-w-2xl"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-violet-300">Live development roster</div><h2 className="mt-3 text-3xl sm:text-4xl font-bold">See what is actually moving.</h2><p className="mt-4 text-sm sm:text-base leading-7 text-slate-400">A public-facing view of VIRORA's work: ideas become planned work, planned work becomes active work, and shipped work becomes part of the system.</p></div><div className="mt-10 grid lg:grid-cols-4 gap-4">{Object.keys(meta).map(type=><Column key={type} type={type} onPick={setPicked}/>)}</div></section>

      <section className="mt-28 relative overflow-hidden rounded-[2rem] border border-violet-400/15 bg-gradient-to-br from-violet-500/[0.09] via-indigo-500/[0.04] to-transparent p-8 sm:p-12"><div className="relative max-w-3xl"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-violet-300">The destination</div><h2 className="mt-3 text-3xl sm:text-5xl font-bold tracking-tight">From separate learning tools to one connected system.</h2><p className="mt-5 text-sm sm:text-base leading-7 text-slate-400">Vocabulary, grammar, practice, assessment, progress and AI should continuously inform one another — so the next thing VIRORA gives you is based on what you actually need.</p><div className="mt-7 flex flex-wrap gap-2">{["Skills","Curriculum","Practice","Vocabulary","Grammar","Progress","AI"].map(x=><span key={x} className="rounded-full border border-white/10 bg-white/[0.035] px-3 py-1.5 text-xs text-slate-300">{x}</span>)}</div></div></section>
    </main>

    {picked && <div className="fixed inset-0 z-50 bg-black/65 backdrop-blur-sm flex items-end sm:items-center justify-center p-4" onClick={() => setPicked(null)}><motion.div initial={{opacity:0,y:18}} animate={{opacity:1,y:0}} className="w-full max-w-lg rounded-3xl border border-white/10 bg-[#101125] shadow-2xl p-6" onClick={event => event.stopPropagation()}><div className={`text-[10px] font-black uppercase tracking-[0.18em] ${meta[picked.type].color}`}>{meta[picked.type].label}</div><h3 className="mt-4 text-2xl font-bold">{picked.title}</h3><p className="mt-3 text-sm leading-6 text-slate-400">{picked.desc}</p><div className="mt-5 rounded-xl border border-white/5 bg-white/[0.025] p-3 text-xs text-slate-500">Roadmap chapter: <span className="text-slate-300">{picked.stage}</span></div><button onClick={() => setPicked(null)} className="mt-5 text-xs font-bold text-violet-300">Close</button></motion.div></div>}
  </div>;
}
