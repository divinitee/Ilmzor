...
export default function TeacherMaterials() {
  ...
  const createMaterial = async () => {
    ...
    try {
      ...
      setFile(null);
      await load();
      if (text.trim()) await processMaterial(row.id);
    } catch (error) {
      console.error(error);
    } finally {
      setSaving(false);
      ...
      <main className="max-w-4xl mx-auto p-4 md:p-6 space-y-6">
        <section className="premium-card p-5 space-y-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Paste text now. File upload is stored and ready for the extraction layer.</p>
          </div>
          ...
          {materials.map((m) => (
            <div key={m.id} className="premium-card p-4 flex items-center justify-between gap-4">
              <div className="min-w-0">
                ...
                <div className="text-xs text-muted-foreground mt-1">{m.source_type.toUpperCase()} · {m.processing_status}</div>
              ...
              <Button variant="outline" size="sm" disabled={processingId === m.id || m.processing_status === "processing" || !m.source_text?.trim()} onClick={() => processMaterial(m.id)}>
