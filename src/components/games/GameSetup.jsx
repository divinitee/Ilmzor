...
export const DIFFICULTIES = [
  { id: "beginner", label: "Beginner", emoji: "🌱", color: "from-emerald-500 to-teal-500" },
  { id: "intermediate", label: "Intermediate", emoji: "🚀", color: "from-sky-500 to-blue-500" },
  { id: "advanced", label: "Advanced", emoji: "🔥", color: "from-violet-500 to-purple-500" },
  { id: "proficient", label: "Proficient", emoji: "👑", color: "from-amber-500 to-orange-500" },
];
    ...
    <div className="max-w-lg mx-auto px-4 py-6">
      <div className="flex items-center justify-between mb-6">
        <button onClick={onCancel} className="text-muted-foreground text-sm hover:text-foreground flex items-center gap-1 select-none">
          <ArrowLeft className="w-4 h-4" /> {t("gameui.cancel")}
        </button>
        <span className="text-xs text-muted-foreground truncate ml-2">{unitName}</span>
      </div>

      <h2 className="text-lg font-bold text-foreground mb-1">{t("gameui.setup_title")}</h2>
      <p className="text-sm text-muted-foreground mb-6">{t("gameui.setup_sub")}</p>

      {/* Difficulty */}
      <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-3">{t("gameui.difficulty_level")}</p>
        ...
        {DIFFICULTIES.map((d, i) => {
          ...
          return (
            ...
            >
              ...
              <p className="text-xs text-muted-foreground">{t(`gameui.diff.${d.id}`)}</p>
      ...
      {isQuiz && (
        <>
          <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-3 flex items-center gap-1">
            <Clock className="w-3.5 h-3.5" /> {t("gameui.time_per_q")}
          </p>
          <div className="flex flex-wrap gap-2 mb-6">
            {TIME_OPTIONS.map(sec => {
              const active = timePerQ === sec;
              return (
                <button
                  key={sec}
          ...
          <div className="flex items-center justify-between bg-background border border-border rounded-2xl p-4 mb-6">
            <div className="pr-3">
              <p className="text-sm font-semibold text-foreground">{t("gameui.auto_advance")}</p>
              <p className="text-xs text-muted-foreground mt-0.5">{t("gameui.auto_advance_desc")}</p>
            </div>
            <button
      ...
      <Button onClick={() => onStart({ difficulty, timePerQ, autoAdvance })} className="w-full h-12 text-base font-bold select-none">
        <Zap className="w-4 h-4 mr-1" /> {t("gameui.start")}
      </Button>
    </div>
  );
}