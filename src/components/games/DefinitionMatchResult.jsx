...
export default function DefinitionMatchResult({ summary, words, accent, onKeepGoing, onPlayAgain, onExit }) {
  const { c, t } = useMatchCopy();
  ...
  return (
    <motion.div initial={rm ? false : { opacity: 0, y: 16, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }} className="premium-card p-5 w-full max-w-sm mx-auto">
      ...
      <p className="text-[11px] text-muted-foreground text-center mt-0.5">
        ...
        {hintMultiplier === 1 && c("example_bonus")}
      </p>
      ...
      <ul className="max-h-36 overflow-y-auto space-y-1 pr-1">
        {words.map((w) => (
          <li key={w.english} className="flex items-center justify-between gap-2 text-xs">
            <span className={`font-semibold truncate ${firstTrySet.has(w.english) ? "text-foreground" : "text-muted-foreground"}`}>{w.english}</span>
            <DefinitionMatchBadge provenance={w._provenance} solved={firstTrySet.has(w.english)} />
          </li>
        ))}
      </ul>
