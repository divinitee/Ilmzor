...
import { ArrowLeft, Star, Flame, Clock } from "lucide-react";
import { useFableCopy } from "@/components/games/cardFlipFableCopy";

// Header chrome: back / title / XP / streak / clock. The clock only renders
// once a board exists (peek + playing), so loading and result stay calm.
export default function CardFlipFableHud({ accent, onBack, xp, streak, secondsLeft, showTimer }) {
  ...
  return (
    <header className="bg-background/70 backdrop-blur-xl border-b border-white/10 px-3 py-2 flex items-center justify-between safe-header gap-2">
      ...
      <div className="flex items-center gap-1.5 text-xs font-bold select-none">
        {showTimer && (
          <span className={`neo-pill px-2.5 h-8 tabular-nums ${low ? "text-rose-300" : "text-foreground/80"}`} aria-label={c("time")} aria-live={low ? "polite" : "off"}>
            <Clock className="w-3.5 h-3.5" aria-hidden="true" /> {secondsLeft}s
          </span>
        )}
