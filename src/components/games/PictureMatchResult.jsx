...
export default function PictureMatchResult({ summary, items, accent, onKeepGoing, onPlayAgain, onExit }) {
  const { c, t } = usePictureCopy();
  const rm = useReducedMotion();
  const { passed, reason, firstTry = [], tries, budget, accuracyPct, streakBest, amount, streakBonus, itemsCorrect, itemsTotal } = summary;
  ...
  return (
    <motion.div initial={rm ? false : { opacity: 0, y: 16, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }} className="premium-card p-5 w-full max-w-sm mx-auto">
      ...
      <div className="grid grid-cols-2 gap-2 mt-4">
        <Stat label={c("matched")} value={`${itemsCorrect}/${itemsTotal}`} />
      ...
      <ul className="max-h-36 overflow-y-auto space-y-1 pr-1">
        {items.map((it) => (
          <li key={it.id} className="flex items-center justify-between gap-2 text-xs">
            <span className={`font-semibold truncate ${firstTrySet.has(it.word) ? "text-foreground" : "text-muted-foreground"}`}>{it.emoji} {it.word}</span>
            <PictureMatchBadge provenance={it.provenance} solved={firstTrySet.has(it.word)} />
          </li>
        ))}
      </ul>
