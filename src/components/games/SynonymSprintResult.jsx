...
import SynonymSprintBadge from "@/components/games/SynonymSprintBadge";
...
export default function SynonymSprintResult({ summary, items, accent, onKeepGoing, onPlayAgain, onExit }) {
      ...
      <ul className="max-h-36 overflow-y-auto space-y-1 pr-1">
        {items.map((it) => (
          <li key={it.id} className="flex items-center justify-between gap-2 text-xs">
            ...
            <SynonymSprintBadge provenance={it.provenance} solved={firstTrySet.has(it.word)} compact />
          </li>
        ))}
      </ul>
